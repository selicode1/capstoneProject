...
export default function ProductsSales1() {
  // 1. Rating Component (for stars)
  const Rating = ({ value }) => {
    return (
  ...
  const ProductCard = ({ product }) => {
    return (
      <div className="relative bg-white p-4 rounded-md shadow text-center">
        {/* Optional Green Label on the left */}
        {product.discountBadge && (
          <div className="absolute left-0 top-1/2 -translate-y-1/2 bg-green-500 text-white px-2 py-1 text-xs font-semibold rounded-r-md">
            {product.discountBadge}
          </div>
        )}
